...
PIN 해시는 클라이언트에서 검증합니다. 즉 `logins` 문서를 읽을 수 있는 사람은 해시를 얻어 4자리(1만 가지)를 오프라인 무차별 대입할 수 있습니다 — Apps Script 버전과 위험도 동일한, 친구 행사 수준에서 감수하는 트레이드오프입니다. **실제로 쓰는 PIN과 겹치지 않게** 안내 문구가 뜨는 이유입니다. 규칙은 "이미 설정된 pinHash 덮어쓰기"를 막아 계정 가로채기는 방지합니다. (PIN을 잊었으면 관리자 패널 "🔑 PIN 관리"에서 그 사람 것만 초기화 — 다음 로그인 때 새로 설정하게 됩니다)
...
4. **데이터 관리** — 콘솔 안 열고 앱에서:
   - **점수 로그 관리** (`#/admin/scores`): 팀/개인 점수 로그를 최신순·실시간으로 보고 비고/점수 수정·삭제 (잘못 넣었을 때)
   - **미션칩 전체 관리** (`#/admin/missions`): 58개 전부를 EZ/MD/HD 그룹으로, 실시간(누가 스캔하면 즉시 반영). 검색, 아무 칩이나 상태 판정, 미션 내용·점수·종목 인라인 수정, 삭제, 새 칩 추가
   - **PIN 관리** (`#/admin/pins`): 사람별 PIN 설정 여부 확인 + 초기화 (초기화하면 그 사람 다음 로그인 때 새 PIN 설정)
